...
func (nc *NotificationController) UpdateServiceProviderFCMToken(c echo.Context) error {
	...
	// Get service provider ID from JWT token (assuming it's set by auth middleware)
	serviceProviderID, ok := c.Get("serviceProviderId").(primitive.ObjectID)
	if !ok {
		return c.JSON(401, map[string]interface{}{
			"success": false,
			"message": "Unauthorized - service provider ID not found",
		})
	}
